...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_c=3, embed_dim=768, norm_layer=None): #embed_dim=16*16*3
        super().__init__()
...
class Attention(nn.Module): #实现注意力
    ...
    def forward(self, x):
        # [batch_size, num_patches + 1, total_embed_dim] #num_patches + 1 加上class token
        B, N, C = x.shape
...
class Block(nn.Module): #实现一个embedblock
    def __init__(self,
                 dim,
                 num_heads,
                 mlp_ratio=4.,
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
                 ...
                 act_layer=None): #embed_layer=PatchEmbed默认为PatchEmbed层结构
        """
        Args:
...
class projection_network(nn.Module): #定义投影层
    def __init__(self, in_feature, hidden_size=128):
        ...
        self.pro_layer2 = nn.Linear(in_feature, hidden_size)
    def forward(self,x):
        x=self.pro_layer1(x)
        x=self.relu1(x)
        x=self.dropout1(x)
...
class two_vit_spatial_unsample(nn.Module):
    def __init__(self, num_classes=2, weight_path=None):
        super(two_vit_spatial_unsample, self).__init__()
        self.spatial= vit_base_patch16_224_in21k()
        self.unsampled= vit_base_patch16_224_in21k()
        if weight_path is not None:
            self.spatial.load_state_dict(torch.load(weight_path))
            self.unsampled.load_state_dict(torch.load(weight_path))
    ...
    def forward(self, x):
        NPR = x - self.interpolate(x, 0.5)  # 先经过插值计算得到上采样伪影

        y_unsample,feature_unsample = self.spatial(NPR * 2.0 / 3.0) #上采样流
        y_spatial,feature_spatial = self.unsampled(x)
        y = torch.cat((feature_unsample[:,0], feature_spatial[:,0]), dim=1)
        return self.classifier(y),y




if __name__ == '__main__':
    model = two_vit_spatial_unsample(num_classes=2)
